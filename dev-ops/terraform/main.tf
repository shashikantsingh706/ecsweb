resource "aws_ecs_cluster" "web_server_cluster" {
  name = "ecs_yz_web_server_cluster"
}
...
resource "aws_ecs_task_definition" "web_server_task" {
  family                   = "ecs-cql-web-server-task"
  ...
  container_definitions = jsonencode([
    {
      name   = "ecs-cql-web-server-container"
...
resource "aws_cloudwatch_log_group" "ecs_log_group" {
  name              = "/ecs/ecs-cql-web-server-task"
  retention_in_days = 7
}

resource "aws_lb_target_group" "web_server_target_group" {
  name        = "ecs-cql-target-group"
  port        = 80
  protocol    = "HTTP"
...
resource "aws_ecs_service" "web_server_service" {
  name             = "ecs-cql-web-server-service"
  ...
  load_balancer {
    ...
    container_name   = "ecs-cql-web-server-container"
