...
resource "aws_ecs_task_definition" "web_server_task" {
  family                   = "${terraform.workspace}-cql-web-server-task"
  ...
  container_definitions = jsonencode([
    {
      name   = "${terraform.workspace}-cql-web-server-container"
...
resource "aws_cloudwatch_log_group" "ecs_log_group" {
  name              = "/ecs/${terraform.workspace}-cql-web-server-task"
  retention_in_days = 7
}

resource "aws_lb_target_group" "web_server_target_group" {
  name        = "${terraform.workspace}-cql-target-group"
  port        = 80
  protocol    = "HTTP"
...
resource "aws_ecs_service" "web_server_service" {
  name             = "${terraform.workspace}-cql-web-server-service"
  ...
  load_balancer {
    ...
    container_name   = "${terraform.workspace}-cql-web-server-container"
...
resource "aws_lb" "web_server_load_balancer" {
  name               = "${terraform.workspace}-cql-load-balancer"
  internal           = false
